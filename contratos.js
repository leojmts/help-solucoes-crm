...
async function excluirParcelasCanceladasContrato(id){
  const c=contratosRegistros.find(x=>x.id===id);if(!c||c.status!=='Cancelado')return avisarModulo('Somente contratos cancelados permitem excluir parcelas.');
  if(!confirm(`Excluir todas as parcelas sem pagamento do contrato ${c.numero}?\n\nPagamentos realizados nunca serão excluídos. Esta ação não pode ser desfeita.`))return;
  const{data,error}=await supabaseClient.rpc('excluir_parcelas_canceladas_contrato',{p_contrato_id:id});
  if(error)return avisarModulo(error.message);const resultado=Array.isArray(data)?data[0]:data;
  await carregarContratos();setTimeout(()=>abrirDetalheContrato(id),100);avisarModulo(`${Number(resultado?.parcelas_excluidas||0)} parcela(s) e ${Number(resultado?.lancamentos_excluidos||0)} lançamento(s) sem pagamento foram excluídos.`);
}
