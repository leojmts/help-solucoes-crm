...
async function cadRenderEquipamentos(){
  ...
  const[e,c]=await Promise.all([supabaseClient.from('equipamentos').select('*,clientes!equipamentos_cliente_id_fkey(nome)').eq('ativo',true).order('criado_em',{ascending:false}),supabaseClient.from('clientes').select('*').order('nome')]);
