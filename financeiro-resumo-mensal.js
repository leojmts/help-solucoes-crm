...
(function () {
  ...
  window.renderizarTabelaFinanceiro=function(){
    ...
    const lista=regs.filter(x=>{
      ...
      const okBusca=!busca||[x.descricao,x.categoria,x.ordens_servico?.numero,x.financeiro_fornecedores?.nome,x.contratos?.numero].join(' ').toLowerCase().includes(busca);
